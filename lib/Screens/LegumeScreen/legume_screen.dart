...
class LegumeScreen extends StatefulWidget {
  const LegumeScreen({Key? key}) : super(key: key);

  @override
  State<LegumeScreen> createState() => _LegumeScreenState();
}
...
class _LegumeScreenState extends State<LegumeScreen> {
  ...
  Future readData() async {
    List<Map> response = await sqflite.readData("SELECT * FROM legume");
    List creditBase = await sqflite.readData("SELECT amount from legume");
    List<Map> totalAmo =
        await sqflite.readData("SELECT * from totalamountlegume");
    persons.addAll(response);
    credit.addAll(creditBase);
    totalAmount.addAll(totalAmo);
    if (totalAmount.length == 0) {
      sqflite.insertData(
          "INSERT INTO 'totalamountlegume' ('totalamount') VALUES ('0')");
      List<Map> totalAmo =
          await sqflite.readData("SELECT * from totalamountlegume");
      totalAmount.addAll(totalAmo);
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        backgroundColor: Color.fromARGB(255, 233, 152, 2),
                                                        ...
                                                        child: ElevatedButton(
                                                          ...
                                                          onPressed: () {
                                                            if (_formKey
                                                                ...
                                                                .validate()) {
                                                              sqflite.updateData(
                                                                  "UPDATE 'totalamountlegume' SET ('totalamount') = ('${totalAmountController.text}') WHERE id = ${1}");
                                                              refresh();
                                                              Navigator.of(
                                                                      context)
                                                                  .pop();
